...
blob_radius = np.array([0.83284136573349932, 0.48710611214400001, 0.26201755389999998, 0.13505535066599994, 0.0684099578379999268])/2.0
...
for ex in range(expNum):
    ...
    for thread in range(ex, expNum*threadsEach, expNum):
...
eta                                      1.0
blob_radius                              ''' + str(blob_radius[ex]) +'''

debye_length                             ''' + str(d) +'''
debye_length_wall                        ''' + str(d-1) +'''
...
structure ''' + str(modelFile[ex]) +''' data/''' + str(thread) +'''/constrained_spheres.clones
            ''')
